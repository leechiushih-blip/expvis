...
// ============ Version ============
//
// The version of ExpVis itself.
//
// Every generated experiment named the jsPsych it targets and said nothing
// about the editor that produced it, so a file could not be matched to the
// build that wrote it. That is the version problem the review raised: a
// published tutorial outlives the software it describes, so a reader has to be
// able to tell which version a tutorial belongs to and how to get that version
// back.
//
// ONE source of truth. The header of every generated file, the version shown in
...
// Default: any-key to continue (for instructions / feedback / stimulus-only).
      ...
      function _htmlHead(usedPlugins, extraStyle, extraHead) {
        ...
        h += '  <title>' + title + '</title>\n';
        h += '  <!--\n';
        h += '    Generated by ExpVis v' + EXPVIS_VERSION + ' on ' + today +
             ' | Device: ' + devName + '\n';
        h += '    Target: jsPsych v' + _JSPsychVersion + '\n';
        // The version on its own is only half of it: the review asked for how to
        // GET that version back, which is the tag on the repository.
        h += '    Editor: ' + EXPVIS_REPO + ' (tag v' + EXPVIS_VERSION + ')\n';
      ...
      // The version in the editor's own header, from the single constant.
      (function () {
        var el = document.getElementById('app-version');
        if (!el) return;
        el.textContent = 'v' + EXPVIS_VERSION;
        el.title = 'ExpVis v' + EXPVIS_VERSION + '\n' + EXPVIS_REPO +
          '\nRetrieve this exact version with: git checkout v' + EXPVIS_VERSION;
      })();
